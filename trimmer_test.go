...
var naughtyStringsBase64 = []string{
	"",
	"dW5kZWZpbmVk",
	"dW5kZWY=",
	"bnVsbA==",
...
var naughtyStrings = func(ss []string, f func(string) string) []string {
	res := make([]string, len(ss))
	for idx, v := range ss {
		res[idx] = f(v)
	}
	return res
}(naughtyStringsBase64, decodeBase64String)
...
var generateTestCases = func(ss []string) []testCase {
	// Source: https://stackoverflow.com/a/23416215
	nextProduct := func(a []rune, r int) func() []rune {
		p := make([]rune, r)
		x := make([]rune, len(p))
		return func() []rune {
			p := p[:len(x)]
	...
	chars := make([]rune, 0)
	cutset.Each(func(value rune) {
		chars = append(chars, value)
	})
	maxLength := cutset.Len()
	prefixesSuffixes := func(chars []rune) []string {
		// create permutations of chars with length of range [0,maxLength]
		// repeated characters are allowed
		res := make([]string, 0)
		np := nextProduct(chars, maxLength)
		for {
			product := np()
			if len(product) == 0 {
				break
			}
			// remove spaces to create prefixes/suffixes of length shorter than maxLength
			productString := strings.ReplaceAll(string(product), " ", "")
			res = append(res, productString)
		}
		return res
	}(chars)
	generatePrefixSuffixPermutations := func(idx int, s string) []testCase {
		res := make([]testCase, 0)
		for _, productString := range prefixesSuffixes {
			res = append(res, testCase{idx, s})                                 // no prefix or suffix
			res = append(res, testCase{idx, s + productString})                 // prefix only
			res = append(res, testCase{idx, productString + s})                 // suffix only
			res = append(res, testCase{idx, productString + s + productString}) // prefix and suffix
		}
		return res
	}
	res := make([]testCase, 0)
	for idx, s := range ss {
		res = append(res, generatePrefixSuffixPermutations(idx, s)...)
	}
	return res
}
...
func BenchmarkTrim(b *testing.B) {
	testCases := generateTestCases(naughtyStrings)
	b.Run(fmt.Sprint("Trim"), func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			for _, s := range testCases {
				strings.Trim(s.str, charsToTrim)
			}
		}
	})
	b.Run(fmt.Sprint("TrimLeft"), func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			for _, s := range testCases {
				strings.TrimLeft(s.str, charsToTrim)
			}
		}
	})
	b.Run(fmt.Sprint("TrimRight"), func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			for _, s := range testCases {
				strings.TrimRight(s.str, charsToTrim)
			}
		}
	})
}
